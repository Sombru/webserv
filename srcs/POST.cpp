...
static bool unchunkBody(std::string &body)
{
	...
	while (pos_in_body < body.length())
	{
		...
		if (chunk_size == 0)
		{ // used as - found the end of the message
			// Expect final "\r\n"
...
HttpResponse POST(HttpRequest request, const ServerConfig &server)
{
	...
	if (request.body.size() > server.clientMaxBodySize)
	...
	std::string upload_dir = (location && !location->uploadDir.empty())
								 ? location->uploadDir
								 : "./uploads";
	std::string filepath;
...
// TESTING POST:
// curl -X POST http://127.0.0.3:8080/uploads/ -d "Hello World!"
